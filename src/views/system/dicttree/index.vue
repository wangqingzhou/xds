    ...
    <div class="search-container">
      <el-form ref="queryFormRef" :model="queryParams" :inline="true">
        ...
        <el-form-item label="字典类别" prop="type">
          <el-input v-model="queryParams.type" placeholder="字典类别" @keyup.enter="handleQuery" />
        </el-form-item>
        ...
        <el-table-column label="操作" fixed="right" align="left" width="200">
          <template #default="scope">
            <el-button
              ...
              size="small"
              icon="plus"
              @click.stop="handleOpenDialog(scope.row.id, undefined, scope.row.type)"
            >
              新增
            </el-button>
            <el-button
              ...
              size="small"
              icon="edit"
              @click.stop="handleOpenDialog(scope.row.parentId, scope.row.id, scope.row.type)"
            >
              编辑
            </el-button>
            <el-button
...
import DeptAPI, { DeptVO, DeptForm, DeptQuery } from "@/api/system/dicttree.api";
...
const rules = reactive({
  parentId: [{ required: true, message: "上级字典不能为空", trigger: "change" }],
 ...
 * @param deptId 字典ID
 */
async function handleOpenDialog(parentId?: string, deptId?: string, dicttype?: string) {
  // 加载字典下拉数据
  ...
  if (deptId) {
    ...
    DeptAPI.getFormData(deptId).then((data) => {
      data.parentId = Number(data.parentId); // 确保是数字
      console.log(data);
      Object.assign(formData, data);
    });
  } else {
    dialog.title = "新增字典";
    formData.parentId = parentId ?? 0; // 使用 nullish 合并运算符
    formData.type = dicttype;
  }
}
