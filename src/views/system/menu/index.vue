    ...
    >
      <el-form ref="menuFormRef" :model="formData" :rules="rules" label-width="100px">
        ...
        <el-form-item v-if="formData.type == MenuTypeEnum.MENU">
          ...
          <div v-if="!formData.paramsList || formData.paramsList.length === 0">
            <el-button type="success" plain @click="formData.paramsList = [{ key: '', value: '' }]">
              添加路由参数
            </el-button>
          </div>

          <div v-else>
            <div v-for="(item, index) in formData.paramsList" :key="index">
              ...
              <el-icon
                v-if="formData.paramsList.indexOf(item) === formData.paramsList.length - 1"
                class="ml-2 cursor-pointer color-[var(--el-color-success)]"
                style="vertical-align: -0.15em"
                @click="formData.paramsList.push({ key: '', value: '' })"
              >
                <CirclePlusFilled />
              </el-icon>
              <el-icon
                ...
                style="vertical-align: -0.15em"
                @click="formData.paramsList.splice(formData.paramsList.indexOf(item), 1)"
              >
                <DeleteFilled />
              </el-icon>
            </div>
 ...
 * 打开表单弹窗
 *
 * @param parentId 父菜单ID（可能为字符串或undefined）
...
function handleOpenDialog(parentId?: string, menuId?: string) {
  ...
  MenuAPI.getOptions(true)
    .then((data) => {
      // 顶级菜单选项：value设为数字0，确保与后端一致
      menuOptions.value = [
        {
          value: 0, // 明确为数字类型，与后端顶级菜单parentId=0匹配
          label: "顶级菜单",
          children: data,
        },
      ];
    ...
    .then(() => {
      ...
      if (menuId) {
        ...
        MenuAPI.getFormData(menuId).then((data) => {
          ...
          // 确保parentId为数字类型（后端通常用数字存储）
          formData.value = {
            ...data,
            parentId: Number(data.parentId), // 强制转为数字，避免字符串类型
          };
        });
      } else {
        // 新增菜单逻辑
        dialog.title = "新增菜单";
        // 关键修复：将parentId转为数字，确保0是数字类型
        // 若parentId为undefined，则默认0（顶级菜单）
        formData.value.parentId = parentId ? Number(parentId) : 0;
      }
    })
    .catch((err) => {
      console.error("打开菜单弹窗失败：", err);
      // 可添加错误提示，如ElMessage.error("加载菜单数据失败")
    });
}
