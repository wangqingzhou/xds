...
const ONLINE_USER_URL = "/api/v1/units/count"; // 在线用户统计接口

// 在线用户统计组合式API（仅页面加载/刷新时调用）
export function useOnlineCount() {
  ...
  const fetchOnlineCount = async () => {
    try {
      const response = await request<any, { count: number }>({
        url: ONLINE_USER_URL,
        method: "get",
      });
      console.log(response);
      onlineUserCount.value = response.count || 0;
  ...
  // 仅在组件挂载时调用一次（页面打开或刷新时）
  onMounted(() => {
    fetchOnlineCount();
  });

  // 暴露手动更新方法（如需在特定场景手动刷新）
  const refresh = () => {
    return fetchOnlineCount();
  };
